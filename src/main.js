...
import { preloadModels, getModelClone, hasModel, getSourceModel } from './core/ModelLoader.js';
import { proceduralRerig } from './core/ProceduralRig.js';
import sceneManager from './rendering/SceneManager.js';
import { createColorMaterial, createIBLMaterial } from './rendering/IBLMaterial.js';
...
import { initSimulation, simulationStep as runSimulationStep } from './systems/SimulationSystem.js';
import { createPlaceholderTerrain, createWaterSurface } from './rendering/SceneSetup.js';
...
function flag(name, desktopDefault = true, mobileDefault = false, vrDefault = undefined) {
...
if (isMobile || isVRDevice || _params.toString()) {
  console.log('[PolyFish] Feature flags:', {
    isMobile, isVRDevice, isIOS, USE_CAUSTICS, USE_SNOW, USE_FULL_SNOW,
    USE_FULL_POOLS, USE_HIRES_TERRAIN, USE_HI_DPR, USE_INSTANCING, USE_DOF,
    USE_GODRAYS, USE_SWIM, USE_WATER_ANIM,
  });
}
...
function buildModeContext() {
  return {
    ...
    audioManager,
    allCreaturePools,
    fishPool, dolphinPool, manateePool,
    foodPool, seedPool, plantPool,
    // Spawn helpers
    spawnCreature,
    spawnFood,
    spawnSeed,
    ...
    get stageTimer() { return GS.stageTimer; },
    set stageTimer(v) { GS.stageTimer = v; },
    get stageRunning() { return GS.stageRunning; },
    set stageRunning(v) { GS.stageRunning = v; },
...
const _diagMode = new URLSearchParams(window.location.search).get('diag') === 'true';
let _diagEl = null;
...
if (_diagMode) {
  window.addEventListener('error', (e) => diagLog(`ERROR: ${e.message} @ ${e.filename}:${e.lineno}`));
  window.addEventListener('unhandledrejection', (e) => diagLog(`REJECT: ${e.reason}`));
}
...
async function init() {
  ...
  diagLog(`SAB: ${typeof SharedArrayBuffer !== 'undefined'} iOS:${isIOS} mobile:${isMobile} noSim:${_params.has('noSim')} minimal:${MINIMAL}`);
  ...
  if (copyrightEl) {
    copyrightEl.textContent += ` | VR ${VR_BUILD_VERSION}`;
  }
  ...
  for (let row = 0; row < terrainSamples; row++) {
    for (let col = 0; col < terrainSamples; col++) {
      const worldX = (TERRAIN_CENTER_X - halfSize) + (col / (terrainSamples - 1)) * TERRAIN_SIZE;
      const worldZ = (TERRAIN_CENTER_Z - halfSize) + (row / (terrainSamples - 1)) * TERRAIN_SIZE;
      terrainHeightData[row * terrainSamples + col] = getTerrainHeight(worldX, worldZ);
    }
  }
  physicsProxy.createTerrainBody(terrainHeightData, terrainSamples, TERRAIN_SIZE, TERRAIN_CENTER_X, TERRAIN_CENTER_Z);
  ...
  // 10c. Player kinematic capsule — pushes food/creatures/seeds aside
  {
    ...
    playerBodySlot = physicsProxy.createBody(
      { type: 'capsule', halfHeight: capsuleHalfHeight, radius: capsuleRadius },
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (devMode) {
      ...
      if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
        ...
        if (e.key === '2') { modeManager.switchMode('model-viewer', modeContext); return; }
        if (e.key === '3') { modeManager.switchMode('editor', modeContext); return; }
      }
  ...
  if (_diagMode) {
    ...
    setInterval(() => {
      ...
      if (_diagFrameCount === lastFrameCount) {
        diagLog(`WATCHDOG: loop stalled at f${_diagFrameCount} ctxLost=${gl.isContextLost()}`);
      }
...
function initPools(scene) {
  ...
  function patchCreatureCaustics(creature) {
    creature.originalMaterials.forEach(({ material }) => applyCaustics(material, 2.0));
  }
  ...
  if (USE_INSTANCING) {
    ...
    if (fishSource) {
      ...
      fishInstanced = createCreatureInstanced(fishSource, 'fish', MAX_FISH_INSTANCES);
      if (fishInstanced) scene.add(fishInstanced.mesh);
    }
    const dolphinSource = getModelClone('dolphin');
    if (dolphinSource) {
      dolphinSource.scale.setScalar(1);
      dolphinInstanced = createCreatureInstanced(dolphinSource, 'dolphin', MAX_DOLPHIN_INSTANCES);
    ...
    if (manateeSource) {
      ...
      manateeInstanced = createCreatureInstanced(manateeSource, 'manatee', MAX_MANATEE_INSTANCES);
  ...
  // Create shared InstancedMesh for all food particles
  {
    ...
    foodInstancedMesh = new THREE.InstancedMesh(foodGeo, foodMat, MAX_FOOD_INSTANCES);
  ...
  // Create shared InstancedMesh for all seeds
  {
    ...
    const seedMat = createColorMaterial(CONFIG.seedColor, { flatShading: true });
    applyCaustics(seedMat, 2.0);
    seedInstancedMesh = new THREE.InstancedMesh(seedGeo, seedMat, MAX_SEED_INSTANCES);
  ...
  if (USE_INSTANCING) {
    ...
    if (kelpSourceForInstancing) {
      plantInstanced = createPlantInstanced(kelpSourceForInstancing, MAX_PLANT_INSTANCES);
...
function spawnCreature(type, position, parentSpeed) {
  const poolMap = { fish: fishPool, dolphin: dolphinPool, manatee: manateePool };
...
function spawnInitialSeed() {
  ...
  const seed = seedPool.get();
  if (!seed) { console.warn('[PolyFish] No seed available in pool!'); return; }
  seed.activate(pos);
...
function gameLoop() {
  ...
  if (_diagMode && _diagFrameCount <= 5) {
    ...
    diagLog(`frame ${_diagFrameCount}: glErr=${glErr} programs=${r.info.programs?.length || 0} ctxLost=${gl.isContextLost()}`);
    if (_diagFrameCount === 1) {
      diagLog(`canvas: ${r.domElement.width}x${r.domElement.height} inDOM=${!!r.domElement.parentNode}`);
      diagLog(`cam: ${sceneManager.getCamera().position.toArray().map(v => v.toFixed(1))}`);
  ...
  if (_params.has('noSim') || MINIMAL) {
    ...
    const camera = sceneManager.getCamera();
    renderer.info.reset();
    if (_diagMode && _diagFrameCount <= 5) diagLog(`f${_diagFrameCount} render(noSim)...`);
    sceneManager.render();
    if (_diagMode && _diagFrameCount <= 5) diagLog(`f${_diagFrameCount} render(noSim) OK`);
    return;
  }
  ...
  vfxManager?.setCreaturePositions(GS._activeFish, GS._activeDolphins, GS._activeManatees);
  ...
  if (_diagMode && _diagFrameCount <= 5) diagLog(`f${_diagFrameCount} sim...`);
  for (let i = 0; i < timeScale; i++) {
    runSimulationStep(rawDt, elapsed);
  }
  if (_diagMode && _diagFrameCount <= 5) diagLog(`f${_diagFrameCount} sim OK`);
  ...
  if (xrManager && xrManager.active) {
    ...
    if (xrResults && Array.isArray(xrResults)) {
      for (let ci = 0; ci < xrResults.length; ci++) {
        ...
        if (r.triggerHeld && !_vrHeldFood[ci] && !_vrFoodSpawned[ci] && r.grip) {
  ...
  const camera = sceneManager.getCamera();
  const populationData = hudSystem.update(dt, { fishPool, dolphinPool, manateePool, plantPool, foodPool, seedPool }, camera);
  ...
  if (_diagMode && _diagFrameCount <= 5) diagLog(`f${_diagFrameCount} render...`);
  if (cinematicDOF && cameraController._dofEnabled && cameraController.mode === 'screensaver') {
    ...
    if (godRayRenderer && godRayRenderer.enabled) godRayRenderer.captureDepth();
    sceneManager.render();
    if (godRayRenderer && godRayRenderer.enabled) godRayRenderer.renderOverlay();
  }
  if (_diagMode && _diagFrameCount <= 5) diagLog(`f${_diagFrameCount} render OK`);
  ...
  if (_diagMode && _diagFrameCount === 3 && !gameLoop._shaderChecked) {
    ...
    diagLog(`render: calls=${info.calls} tris=${info.triangles} pts=${info.points}`);
    ...
    for (const prog of programs) {
      if (prog.diagnostics && !prog.diagnostics.runnable) {
        shaderErrors++;
        diagLog(`SHADER FAIL: ${prog.name} — ${prog.diagnostics.fragmentShader?.log || prog.diagnostics.vertexShader?.log || 'unknown'}`);
      }
    }
...
function restartEcosystem() {
  ...
  if (foodInstancedMesh) { scene.remove(foodInstancedMesh); foodInstancedMesh = null; }
  if (seedInstancedMesh) { scene.remove(seedInstancedMesh); seedInstancedMesh = null; }
  if (plantInstanced) { scene.remove(plantInstanced.mesh); plantInstanced = null; }
  if (fishInstanced) { scene.remove(fishInstanced.mesh); fishInstanced = null; }
  if (dolphinInstanced) { scene.remove(dolphinInstanced.mesh); dolphinInstanced = null; }
  if (manateeInstanced) { scene.remove(manateeInstanced.mesh); manateeInstanced = null; }
